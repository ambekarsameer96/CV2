...
def ICP(source, target, th=0.001, matching_fn=sm.min_dist, sampling_fn=None, iterative_sampling_fn=None):
    ...
    for _ in range(iterations):
        if iterative_sampling_fn is not None:
            source_sampled = iterative_sampling_fn(source, RMS_li)
    ...
    print(f"Mean RMS: {np.mean(RMS_li), {RMS}}")
    return Rt, RMS_li

def run_ICP(source, target, args=dict(), o3dvis=False, matplotvis=True, title='a'):
...
#
# # Using all points
# RMS_li = run_ICP(source, target, title='all')
#
# # Uniform sub-sampling
# unif_args = {'sampling_fn': sm.unif_sampling}
# RMS_li = run_ICP(source, target, args=unif_args, title='unif')
#
# # Random sub-sampling
# rand_args = {'iterative_sampling_fn': sm.unif_sampling}
# RMS_li = run_ICP(source, target, args=rand_args, title='random')
#
# # Multi-resolution sub-sampling
# multires_args = {'iterative_sampling_fn': sm.multires_sampling}
# RMS_li = run_ICP(source, target, args=multires_args, title='multi')
#
# # Informative region sub-sampling
# IR_args = {'sampling_fn': sm.density_sampling}
# RMS_li = run_ICP(source, target, args=IR_args, title='informative')
#
# # KDTree
# KD_args = {'matching_fn': sm.kd_method}
# RMS_li = run_ICP(source, target, args=KD_args, title='KDTree')
#
# # z-buffer
# ZB_args = {'matching_fn': sm.z_buffer}
# RMS_li = run_ICP(source, target, args=ZB_args, title='z-buffer')
...
for i in range(steps, len(file_list), steps):
    ...
    Rt, RMS = ICP(source, target, matching_fn=sm.kd_method, sampling_fn=sm.step_sampling)
    Rts.append(Rt)
    source = target
