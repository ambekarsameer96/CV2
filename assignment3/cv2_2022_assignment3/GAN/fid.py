...
def Test_fid(G, model_path):
    with torch.no_grad():
        G, _, _ = utils.loadModels(G, model_path)
        print('\n Using model path')
        G.to(device) 
        G.eval()
        ...
        pbar = tqdm(enumerate(testLoader), total=batches_train, leave=False)

        Epoch_time = time.time()
        fol1 = os.path.dirname(model_path)
        fol2 = os.path.join(fol1, 'generated')
        if not(os.path.exists(fol2)):
            os.makedirs(fol2)
        print('Saving all images to ', fol2)
        ...
        for i, data in pbar:
            ...
            blend = alpha_blending(source, target, mask)
            pred = G(img_transfer_input)
            #sample_fname, _ = testLoader.dataset.samples[i]
            fname = str(i) + '.jpg'
            f1= os.path.join(fol2, fname)
            print('I am saving to ', f1)
            save_image(pred, f1)

        #save the image 
        print('Done all the images')
...
#Test_fid(G, model_path= '/home/lcur1547/cv2/gan/Exp_Blender_alpha/checkpoints/checkpoint_G_20.pth')
#Test_fid(G, model_path= '/home/lcur1547/cv2/gan/Exp_Blender_no_disc_new/checkpoints/checkpoint_G_20.pth')
Test_fid(G, model_path= '/home/lcur1547/cv2/gan/Exp_Blender_PS/checkpoints/checkpoint_G_18.pth')