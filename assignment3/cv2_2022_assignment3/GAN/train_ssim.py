...
experiment_name = 'Blender_alpha_SSIM'
...
def Train(G, D, epoch_count, iter_count):
    ...
    for i, data in pbar:
        ...
        #add ssim loss here between y and G 
        ...
        _ssim_loss = 1-ssim_loss(output_G, y)
        _ssim_loss.backward()
    ...
    # mean_ssim = ssim_tot / len(dataloader)
    # print('SSIM for epoch', mean_ssim)
    print('[I] STATUS: Epoch %d/%d, Iter %d/%d, Loss_G: %.4f, Loss_D: %.4f, SSIM loss: %.4f, Time: %.4f' %
            (epoch_count, max_epochs, iter_count, batches_train, loss_G.item(), loss_D.item(), ssim_tot/len(trainLoader),  time.time() - Epoch_time))
    trainLogger.write('%d, %d, %d, %.4f, %.4f, %.4f, %.4f\n' %
            (epoch_count, max_epochs, iter_count, loss_G.item(), loss_D.item(), ssim_tot/len(trainLoader), time.time() - Epoch_time))
    print('\n SSIM', ssim_tot/len(trainLoader))
